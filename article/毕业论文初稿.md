...
| 模型                                                | 推理设置   | 图像数 |  GT | Pred |  TP |   FP | FN | Precision | Recall |    F1 | FP/图 | Split Rate | Avg. Fragments/Hit |
| --------------------------------------------------- | ---------- | -----: | --: | ---: | --: | ---: | -: | --------: | -----: | ----: | ----: | ---------: | -----------------: |
| `graphene_mid_finetune_baseline_v7`               | 无后处理   |     40 | 187 | 3950 | 160 | 3790 | 27 |      4.1% |  85.6% |  7.7% | 94.75 |      75.4% |              10.20 |
| `graphene_mid_finetune_baseline_v7`               | 本文后处理 |     40 | 187 |  214 | 121 |   93 | 66 |     56.5% |  64.7% | 60.3% |  2.33 |       0.5% |               1.01 |
| `graphene_mid_finetune_remove_small_flakes_obj20` | 无后处理   |     40 | 187 |  734 | 139 |  595 | 48 |     18.9% |  74.3% | 30.2% | 14.88 |      38.5% |               2.21 |
| `graphene_mid_finetune_remove_small_flakes_obj20` | 本文后处理 |     40 | 187 |  188 | 126 |   62 | 61 |     67.0% |  67.4% | 67.2% |  1.55 |       1.6% |               1.02 |
...
分割可视化与实例级指标位于 `training_log/eval_vis`。其中，`baseline_raw_no_postprocess` 对应 `graphene_mid_finetune_baseline_v7` 无后处理输出，`baseline_raw_with_postprocess` 对应 `graphene_mid_finetune_baseline_v7` 加本文后处理输出，`detailed_test_base_config_no_postprocess` 对应 `graphene_mid_finetune_remove_small_flakes_obj20` 无后处理输出，`detailed_test_base_config` 对应 `graphene_mid_finetune_remove_small_flakes_obj20` 加本文后处理输出。从可视化观察，改进后的模型能够较好捕捉肉眼可见的石墨烯 flake，尤其是边界较清晰、面积较大的 flake。后处理对重复 mask 的合并和小碎片过滤也使输出更接近人工标注。
...
如果需要展示后处理各阶段结果，可进一步给出 raw prediction、overlap merged 和 final result 的调试图。本文当前定量结果主要来自 overlap merge 与 final filtering，即合并重复 mask 后再按 score、面积和形状复杂度过滤。
...
<!--
后续待办：
1. 从 maskterial.pdf 和 green channel.pdf 补全参考文献条目，并把引用编号插入正文。
2. 按学校 Word 模板调整标题样式、页眉页脚、目录页码、图表编号和公式编号。
3. 把最终采用的显微图像整理到 article/figures/，统一命名。
-->
